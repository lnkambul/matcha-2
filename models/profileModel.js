...
Profile.validate = (profile, callback)  => {
	...
	S.locate(profile.city, (err, res) => {e.city = err})
	S.locate(profile.country, (err, res) => {e.country = err})
	S.bio(profile.bio, (err, res) => {e.bio = err})
	if (e.age || e.gender || /*e.sexual_orientation ||*/ e.preference || e.interests || e.city || e.country || e.bio)
		callback(e, null)
	else 
